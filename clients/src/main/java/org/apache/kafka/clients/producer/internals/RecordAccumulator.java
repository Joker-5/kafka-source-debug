...
public final class RecordAccumulator {
     ...
     */
    public RecordAppendResult append(TopicPartition tp, // topic和分区信息，即发送到哪个topic分区
                                     long timestamp, // 客户端发送时的时间戳
                                     byte[] key, // 消息的key
                                     byte[] value, // 消息体
                                     Header[] headers, // 消息头，即额外消息属性
                                     Callback callback, // 回调方法
                                     long maxTimeToBlock, // 消息append的最大超时时间
        ...
        try {
            ...
            /*
             * 关于KafkaDeque的结构：
             * Deque的每一个元素用ProducerBatch表示
             * 每一个ProducerBatch代表一个「batch.size」字节的内存
             * 在kafka中代表一个批次
             */
            // 尝试根据topic与分区在kafka中获取一个deque，如果不存在就创建一个
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                // 调用tryAppend方法将消息append到缓存中
                // Kafka会为每一个topic的每一个分区创建一个消息缓冲区，消息先追加到缓存中，然后消息发送API立即返回，
                // 然后由单独的线程Sender将缓冲区中的消息定时发送到broker
                // 其中缓冲区的实现用的是ArrayDeque
                // 然后调用tryAppend方法尝试将消息追加到其缓冲区
            ...
            // 如果在上面tryAppend没有追加成功，说明当前没有可用的ProducerBatch，
            // 那么此时需要创建一个ProducerBatch
            // 因此需要先从BufferPool中申请batch.size大小的内存空间，为创建ProducerBatch做准备
            byte maxUsableMagic = apiVersions.maxUsableProduceMagic();
            int size = Math.max(this.batchSize, AbstractRecords.estimateSizeInBytesUpperBound(maxUsableMagic, compression, key, value, headers));
            log.trace("Allocating a new {} byte message buffer for topic {} partition {} with remaining timeout {}ms", size, tp.topic(), tp.partition(), maxTimeToBlock);
            // 如果由于BufferPool中剩余内存量不够的话，则最多等待maxTimeToBlock的时间
            // 如果在指定时间内还没能申请到内存，就抛异常
            ...
            synchronized (dq) {
                ...
                // 创建一个新的ProducerBatch，其内部持有一个MemoryRecordsBuilder对象，
                // 该对象负责将消息写入内存中，即写入到ProducerBatch内部持有的内存中，大小为「batch.size」
                ...
                // 将新创建的ProducerBatch添加到Deque的末尾
                dq.addLast(batch);
                // 将该批次加入到incomplete容器中，
                // 该容器存放未完成发送到broker中的消息batch，当Sender线程将消息发送到broker后，则会将其从中移除并释放所占内存
